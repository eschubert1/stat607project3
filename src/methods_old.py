...
def logistic_irls_old(y, X, tol=1e-8, max_iter=1000):
    N = np.shape(X)[0]
    n = np.shape(X)[1]
...
def empirical_bayes_old(y, X, ml_coefs, ml_cov, tol=1e-8, max_iter=100):
    """
...
def semi_bayes_old(y, X, ml_coefs, ml_cov, tau_guess):
    """
...
def preliminary_testing_old(y, X, ml_coefs, ml_covs, alpha=0.10):
    """
...
def compute_estimates_old(y, X, tau_guess, tol=1e-8, max_iter=100, alpha=0.10):
    """
